...
export async function GET() {
  ...
  const mapped = await Promise.all((reports||[]).map(async (r:any) => {
    ...
    const a=r.eso_attachments?.[0];
    if(a){const {data}=await db.storage.from('eso-attachments').createSignedUrl(a.storage_path,3600);imageData=data?.signedUrl}
    return {id:r.id,esoNo:r.report_no,reporterId:r.reporter_id,createdAt:r.reported_at,location:r.locations?.name||'Unknown',category:r.category==='environmental'?'Environmental':'Safety',urgency:r.urgency[0].toUpperCase()+r.urgency.slice(1),description:r.description,imageData,imageName:a?.file_name,status:r.status==='completed'||r.status==='closed'?'Completed':r.status==='in_progress'||r.status==='assigned'||r.status==='waiting'?'In Progress':'Open',assignedTo:r.maintenance_tasks?.[0]?.assigned_to||undefined,correctiveAction:r.corrective_actions?.at(-1)?.action_text,completedAt:r.completed_at};
  }));
  const current=users.find((u:any)=>u.id===me.id);
  return NextResponse.json({currentUser:current,users:canAdmin(me.role)||canMaintain(me.role)?users:[current],reports:mapped,departments:(deps||[]).map((d:any)=>({id:d.id,name:d.name,active:d.active})),locations:(locs||[]).map((l:any)=>({id:l.id,name:l.name}))});
}
