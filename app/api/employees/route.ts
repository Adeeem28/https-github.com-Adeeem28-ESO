...
const roleMap:any={'Employee':'employee','Maintenance':'maintenance','Supervisor':'supervisor','Admin':'admin','Super Admin':'super_admin'};
...
export async function POST(req:Request){
  ...
  const n=names(b.name);const dep=await db.from('departments').select('id').eq('name',b.department).single();
  const {data,error}=await db.rpc('create_eso_employee',{p_employee_no:String(b.employeeId||'').trim(),p_first_name:n.first,p_last_name:n.last,p_department_id:dep.data?.id||null,p_role:roleMap[b.role]||'employee',p_password:b.password||null,p_target:Number(b.annualTarget||12)});
  if(error)return NextResponse.json({error:error.message},{status:400});return NextResponse.json({ok:true,data});
}
