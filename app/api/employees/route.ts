...
export async function POST(req:Request){
  ...
  const n=names(b.name);const dep=await db.from('departments').select('id').eq('company_id',me.company_id).eq('name',b.department).single();const dbRole=roleMap[b.role]||'employee';
  ...
  const {data,error}=await db.from('employees').insert({company_id:me.company_id,employee_no:String(b.employeeId||'').trim(),first_name:n.first,last_name:n.last,department_id:dep.data?.id||null,role:dbRole,annual_eso_target:Number(b.annualTarget||12),active:true,password_hash:null}).select('id').single();
...
export async function PATCH(req:Request){
  ...
  const {data:target}=await db.from('employees').select('id,role').eq('company_id',me.company_id).eq('id',b.id).single();
  ...
  if(b.department){const {data:dep}=await db.from('departments').select('id').eq('company_id',me.company_id).eq('name',b.department).single();updates.department_id=dep?.id||null;}
  ...
  const {error}=await db.from('employees').update(updates).eq('company_id',me.company_id).eq('id',b.id);if(error)return NextResponse.json({error:error.message},{status:400});
...
export async function DELETE(req:Request){
  ...
  const {data:target}=await db.from('employees').select('id,role').eq('company_id',me.company_id).eq('id',b.id).single();if(!target||!canManageTarget(me,target.role))return NextResponse.json({error:'You cannot delete this user.'},{status:403});
  const {error}=await db.from('employees').update({active:false}).eq('company_id',me.company_id).eq('id',b.id);if(error)return NextResponse.json({error:error.message},{status:400});
