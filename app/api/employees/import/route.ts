...
export async function POST(req:Request){
 ...
 const {data:deps}=await db.from('departments').select('id,name').eq('company_id',me.company_id).eq('active',true);const depMap=new Map((deps||[]).map((d:any)=>[d.name.toLowerCase(),d.id]));
 ...
 for(let i=0;i<rows.length;i++){
   ...
   const {data:existing}=await db.from('employees').select('id,role').eq('company_id',me.company_id).eq('employee_no',employeeNo).maybeSingle();
   if(existing){
     ...
     const {error}=await db.from('employees').update(update).eq('company_id',me.company_id).eq('id',existing.id);if(error){errors.push({row:i+2,error:error.message});continue}
     ...
     const {data:newUser,error}=await db.from('employees').insert({company_id:me.company_id,employee_no:employeeNo,first_name:n.first,last_name:n.last,department_id:depId,role,annual_eso_target:Number.isFinite(target)?target:12,active:true,password_hash:null}).select('id').single();if(error){errors.push({row:i+2,error:error.message});continue}
