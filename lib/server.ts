...
export async function platformOwnerSession(){const raw=(await cookies()).get('eso_platform_session')?.value;if(!raw)return null;const[id,sig]=raw.split('.');if(!id||!sig)return null;const expected=sign(`platform:${id}`);if(sig.length!==expected.length||!crypto.timingSafeEqual(Buffer.from(sig),Buffer.from(expected)))return null;const{data}=await db.from('platform_owners').select('id,username,display_name,active').eq('id',id).single();return data?.active?data:null;}
