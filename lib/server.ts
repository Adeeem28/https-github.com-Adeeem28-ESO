...
export function roleName(role:string){return role==='super_admin'?'Super Admin':role==='admin'?'Admin':role==='maintenance'?'Maintenance':'Employee'}
export function canAdmin(role:string){return role==='admin'||role==='super_admin'}
export function canMaintain(role:string){return role==='maintenance'||canAdmin(role)}
