...
export interface User { id:string; employeeId:string; name:string; department:string; role:Role; annualTarget:number; active:boolean; }
export interface ESOReport { id:string; esoNo:string; reporterId:string; createdAt:string; location:string; category:"Safety"|"Environmental"; urgency:Urgency; description:string; imageData?:string; imageName?:string; completionImageData?:string; completionImageName?:string; status:ESOStatus; assignedTo?:string; correctiveAction?:string; completedAt?:string; taskStatus?:string; }
